  ...
  for (const directory of lookup(cwd)) {
    ...
    for (const lock of Object.keys(LOCKS)) {
      if (fileExistsSync(path.join(directory, lock))) {
        const name = LOCKS[lock];
        const result = parsePackageJsonSync(
          path.join(directory, "package.json"),
          onUnknown,
        );
        if (result) {
...
async function parsePackageJson(
  ...
  return !filepath || !(await fileExists(filepath))
    ? null
    : handlePackageManager(filepath, onUnknown);
}
...
function parsePackageJsonSync(
  ...
  return !filepath || !fileExistsSync(filepath)
    ? null
    : handlePackageManager(filepath, onUnknown);
}
