...
async function detectPackageManagers(
  ...
  if (hasNpmrc) {
    detected.push({ packageManager: "npm", source: "config" });
  }
  ...
  if (hasBunConfig) {
    detected.push({ packageManager: "bun", source: "config" });
  }
 ...
 */
export async function isBunPM(): Promise<boolean> {
  const pkgManager = await getUserPkgManager();
  return pkgManager.packageManager === "bun";
}
